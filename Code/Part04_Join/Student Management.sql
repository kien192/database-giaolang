...
SELECT t.*, m.MajorName, m.Hotline FROM Student t JOIN  Major m				--9rows
			ON t.MajorID = m.MajorID					


SELECT t.*, m.MajorName, m.Hotline FROM Student t RIGHT JOIN  Major m       --10rows
			ON t.MajorID = m.MajorID
...
SELECT s.*, m.MajorName, m.Hotline FROM Student s FULL JOIN Major m
		ON s.MajorID = m.MajorID 
		WHERE s.MajorID IN ('SE', 'IA')				--6 rows

SELECT s.*, m.MajorName, m.Hotline FROM Student s FULL JOIN Major m
		ON s.MajorID = m.MajorID 
		WHERE s.MajorID = 'IA' OR s.MajorID = 'SE'		--6 rows , cách tối ưu.


SELECT s.*, m.MajorName, m.Hotline FROM Student s , Major m
					WHERE s.MajorID = m.MajorID AND s.MajorID IN ('SE', 'IA')  --6 rows
...
SELECT t.*, m.MajorName, m.Hotline FROM Student t RIGHT JOIN Major m
		ON t.MajorID = m.MajorID

		SELECT MajorID, COUNT(*) AS [No students] FROM Student GROUP BY MajorID  -- 4
		SELECT MajorID, COUNT(MajorID) AS [No students] FROM Student GROUP BY MajorID --4
...
SELECT m.MajorID, m.MajorName, COUNT(s.MajorID) FROM Student s RIGHT JOIN Major m
				ON s.MajorID = m.MajorID
				GROUP BY m.MajorID, m.MajorName					--CHUẨN NHỨT.
